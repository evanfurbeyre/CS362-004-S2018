...
int buyCard (int supplyPos, struct gameState *state)
{
	...
	{
		if(DEBUG)printf("You do not have enough money to buy that. You have %d coins.\n", state->coins);
		return -1;
	...
	else
	{
		 ...
		 * 
		 */
		gainCard(supplyPos, state, 0, who); /* card goes in discard, this might be wrong.. (2 means goes into hand, 0
											 * goes into discard) */
		...
		if(DEBUG)
		{
			printf
				(
					"You bought card number %d for %d coins. You now have %d buys and %d coins.\n", supplyPos, getCost
						(supplyPos), state->numBuys, state->coins
				);
		}
...
int minionEffect(struct gameState *state, int choice1, int choice2, int handPos)
{
	int currentPlayer = whoseTurn(state);
	int i;
	int j;
...
int cardEffect (int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	case minion:
		return minionEffect(state, choice1, choice2, handPos);
